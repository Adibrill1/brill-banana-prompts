...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') return json(req, res, 200, { admin: auth.isAdmin(req), premium: !!auth.readSession(req, 'license') });
  if (req.method === 'DELETE') {
    auth.clearSession(req, res, req.query.role === 'admin' ? 'admin' : 'license');
  ...
  if (attempt.count >= 10) return json(req, res, 429, { error: 'יותר מדי ניסיונות. נסו שוב בעוד רבע שעה.' });
  try {
    ...
    attempts.delete(ip);
    return json(req, res, 200, { ok: true });
  } catch (error) { return json(req, res, error.status || 503, { error: error.message }); }
};
