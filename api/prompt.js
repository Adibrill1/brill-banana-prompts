...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (license) {
      ...
      premium ||= !!entry && (!entry.expiresAt || Date.parse(entry.expiresAt) > Date.now());
    }
    const readable = catalog.canRead(state, card, premium);
    const version = state._publishedAt || revision;
    return json(req, res, 200, { ...catalog.summarize(card, version), images: card.images.map(src => catalog.imageUrl(src, 1600, version)), prompt: readable ? card.prompt : null, locked: !readable, copyEnabled: readable && catalog.mode(state) !== 'catalog' });
  } catch { return json(req, res, 503, { error: 'לא ניתן לטעון את הפרומפט כרגע' }); }
};
