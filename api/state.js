...
module.exports = async (req, res) => {
  ...
  try {
    const { state, revision } = await repository.read('state.json', { fresh: true });
    return json(req, res, 200, { ...state, _revision: revision, _originals: originals });
  } catch { return json(req, res, 503, { error: 'לא ניתן לטעון נתונים מעודכנים. נסו שוב לפני עריכה.' }); }
};
