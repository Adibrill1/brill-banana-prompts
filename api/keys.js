...
module.exports = async function handler(req, res) {
  ...
  if (getResult.status !== 200 || !getResult.data.content) {
    res.status(500).json({ error: "Could not load keys.json" });
    return;
  }
  var sha = getResult.data.sha;
  var keysData = JSON.parse(
    Buffer.from(getResult.data.content, "base64").toString("utf8"),
  );
  ...
  if (req.method === "POST") {
    ...
    keys[key] = entry;
    keysData.keys = keys;
    var encoded = Buffer.from(JSON.stringify(keysData, null, 2)).toString(
      "base64",
    );
    var putResult = await ghPut(
      ghToken,
      "keys.json",
      encoded,
      sha,
      "Add license key: " + (name || key),
    );
  ...
  if (req.method === "DELETE") {
    ...
    delete keys[delKey];
    keysData.keys = keys;
    var encoded2 = Buffer.from(JSON.stringify(keysData, null, 2)).toString(
      "base64",
    );
    var putResult2 = await ghPut(
      ghToken,
      "keys.json",
      encoded2,
      sha,
      "Remove license key: " + delKey,
    );
