...
for (const file of ['admin.html', 'services.html', 'og-image.jpg']) fs.copyFileSync(file, 'dist/' + file);
...
for (const file of ['state.json', 'keys.json', 'content']) {
  if (fs.existsSync('dist/' + file)) throw new Error('Private data found in public build: ' + file);
}
