...
export async function uploadPendingImages(state, services, { compress, upload, onProgress = () => {} }) {
  ...
  for (const key of Object.keys(state.customImgs || {})) add(state.customImgs, key);
  for (const card of state.added || []) for (let i = 0; i < (card.images || []).length; i++) add(card.images, i);
  ...
  for (const [original, targets] of pending) {
    ...
    if (src.length > 3000000) throw new Error('התמונה גדולה מדי להעלאה. יש לבחור תמונה קטנה יותר.');
    const url = await upload('upload_' + index++, src);
    if (typeof url !== 'string' || !url.startsWith('https://')) throw new Error('השרת לא אישר את העלאת התמונה');
    // Do not overwrite a newer edit made while the upload was pending.
    for (const { object, key } of targets) if (object[key] === original) object[key] = url;
  }
}
