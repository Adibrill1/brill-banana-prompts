...
const AccessDialog = lazy(() =>
  import("./AccessDialog").then((m) => ({ default: m.AccessDialog })),
);
function fromUrl(): Selection {
  ...
  return {
    ...
    page: Math.max(1, Number.parseInt(q.get("page") || "1") || 1),
  ...
  return value && typeof value === "object" && !Array.isArray(value)
    ? Object.fromEntries(
        Object.entries(value).filter(
          ([key, on]) => /^[\w-]{1,160}$/.test(key) && on === true,
        ),
      )
    : {};
}
export default function App() {
  ...
  function navigate(next: Partial<Selection>, replace = false) {
    const value = { ...selectionRef.current, ...next };
    ...
    if (value.favorites) params.set("favorites", "1");
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (query !== selection.q) navigate({ q: query, page: 1 }, true);
    }, 250);
  ...
  useEffect(() => {
    ...
    request<{ premium: boolean }>("/api/session")
      .then(async (session) => {
        if (session.premium && !activationKey) {
          if (active) setPremium(true);
          return;
        }
        const license = activationKey
          ? { key: activationKey }
          : stored<{ key?: string } | null>("brill_license", null);
        if (!license?.key) return;
        const result = await request<{ valid: boolean; expiresAt: string }>(
          "/api/license",
          {
            method: "POST",
        ...
        if (active && result.valid) {
          setPremium(true);
          store("brill_license", { ...license, expiresAt: result.expiresAt });
        }
      })
      .catch(() => {
        /* Public browsing remains available if license validation is offline. */
      });
    return () => {
      active = false;
      clearTimeout(noticeTimer.current);
  ...
  function toggleFavorite(card: Card) {
    setFavorites((current) => {
      const next = { ...current };
      if (next[card.key]) delete next[card.key];
      else next[card.key] = true;
      store("bb_favs", next);
      return next;
    });
  }
  ...
  const activeIndex =
    catalog?.items.findIndex((p) => p.key === opened?.key) ?? -1;
  const category =
    config?.categories.find((c) => c.id === selection.category)?.label ||
    (selection.category === "free"
      ? "חינם"
      : selection.category === "paid"
        ? "בתשלום"
        : "כל הפרומפטים");
  async function shareFavorites(
    kind: "favsPrefix" | "orderMsg" | "sessionMsg",
  ) {
    if (!config) return;
    if (favoriteKeys.length === 0) {
      notify("בחרו תמונות במועדפים כדי לשתף אותן");
      return;
    }
    const phone = (config.wa.phone || "972523919350").replace(/\D/g, "");
    const prefix = (config.wa[kind] || "בחרתי {n} תמונות:").replace(
      "{n}",
      String(favoriteKeys.length),
    );
    ...
    const text =
      ...
      favoriteKeys
        ...
        .join("\n");
    if (text.length > 6000) {
      notify("הרשימה ארוכה לשיתוף אחד. אפשר להעתיק את הרשימה.");
      try {
        await navigator.clipboard.writeText(text);
        notify("רשימת המועדפים הועתקה");
      } catch {
        notify("לא ניתן להעתיק כרגע");
      }
      return;
    }
    window.open(
  ...
  useEffect(() => {
    ...
    if (!key) return;
    const controller = new AbortController();
    loadDetail(key, controller.signal)
      .then((detail) => setOpened(detail))
      .catch((error) => {
  ...
  return (
    <>
      ...
      <header className="site-header">
        <a className="brand" href="/" aria-label="Brill Studio — דף הבית">
          <span className="brand-mark" aria-hidden="true">
            B<span>·</span>
          </span>
          <span>{config?.header.title || "Brill Studio"}</span>
        </a>
        <div className="header-actions">
          {config?.shareWhatsApp && (
            <a className="services-link" href="/services">
              שירותי הסטודיו
            </a>
          )}
          <button
            className={"premium-button " + (premium ? "active" : "")}
            onClick={() => setAccess(true)}
          >
            ◇ <span>{premium ? "החשבון שלי" : "פרימיום"}</span>
          </button>
          <button
      ...
      <main>
        <section className="gallery-heading" aria-label="חיפוש בגלריה">
          <div>
            <p className="eyebrow">THE PROMPT COLLECTION</p>
            <h1>{config?.header.sub1 || "קטלוג ליצירת תמונות אישיות"}</h1>
            <p className="intro">
              {config?.header.sub2 ||
                "בחרו רעיון. העתיקו פרומפט. צרו משהו משלכם."}
            </p>
          </div>
          <form
          ...
          >
            ...
            <input
              ...
              placeholder="מה ניצור היום?"
              autoComplete="off"
            />
            <kbd aria-hidden="true">חיפוש</kbd>
          </form>
        </section>
        <section className="browse-controls" aria-label="סינון ותצוגה">
          <nav className="categories" aria-label="קטגוריות">
            {[
              ...
              <button
                key={c.id}
                aria-pressed={selection.category === c.id}
                className={selection.category === c.id ? "active" : ""}
                onClick={() => navigate({ category: c.id, page: 1 })}
              >
                {c.label}
              </button>
            ))}
          ...
          <div className="results-bar">
            <div className="result-label">
              ...
              <span>{selection.favorites ? "המועדפים שלי" : category}</span>
              <span className="result-count" aria-live="polite">
                {loading ? "…" : catalog?.total.toLocaleString("he-IL")}
              </span>
            </div>
            <div className="view-actions">
              <button
                className={
                  "favorites-filter " + (selection.favorites ? "active" : "")
                }
                aria-pressed={selection.favorites}
                onClick={() =>
                  navigate({ favorites: !selection.favorites, page: 1 })
                }
              >
          ...
          {selection.favorites && config?.shareWhatsApp && (
            <div className="favorites-actions">
              <button onClick={() => shareFavorites("favsPrefix")}>
                שיתוף הבחירה
              </button>
              <button onClick={() => shareFavorites("orderMsg")}>
                הזמנת תמונות
              </button>
              <button onClick={() => shareFavorites("sessionMsg")}>
                קביעת סשן
              </button>
            </div>
          )}
        ...
        {catalog?.stale && (
          <p className="connection-note" role="status">
            מוצג העותק האחרון שנשמר. העדכונים החדשים יופיעו כשהחיבור יחזור.
          </p>
        )}
        <div
          ref={resultsRef}
        ...
        >
          {error ? (
            <div className="empty-state" role="alert">
              <h2>לא הצלחנו לטעון את הגלריה</h2>
              <p>{error}</p>
              <button
            ...
            <div className="initial-loading" role="status">
              <span className="loader" /> טוען את הגלריה…
            </div>
          ) : catalog.total === 0 ? (
            <div className="empty-state">
              ...
              <h2>
                {selection.favorites
                  ? "כאן יופיעו הרעיונות ששמרתם"
                  : "לא נמצאו פרומפטים"}
              </h2>
              <p>
                {selection.favorites
                  ? "סמנו לב ליד תמונה שאהבתם."
                  : "נסו מילת חיפוש אחרת או קטגוריה נוספת."}
              </p>
              <button
                onClick={() => {
                  setQuery("");
                  navigate({ q: "", category: "", favorites: false, page: 1 });
                }}
              >
                חזרה לכל הפרומפטים
              </button>
        ...
        {catalog && catalog.total > 0 && (
          <nav className="pagination" aria-label="עמודי גלריה">
            <button
              disabled={loading || catalog.page <= 1}
              onClick={() => {
                navigate({ page: catalog.page - 1 });
            ...
            <span>
              עמוד <strong>{catalog.page}</strong> מתוך {catalog.pages}
            </span>
            <button
              disabled={loading || catalog.page >= catalog.pages}
              onClick={() => {
                navigate({ page: catalog.page + 1 });
                resultsRef.current?.scrollIntoView({ block: "start" });
                resultsRef.current?.focus({ preventScroll: true });
      ...
      <footer>
        <a className="footer-brand" href="/">
          Brill Studio<span>·</span>
        </a>
        <span>
          {catalog?.catalogTotal.toLocaleString("he-IL") || ""} רעיונות ליצירה
          הבאה שלך
        </span>
      ...
      <Suspense fallback={null}>
        {opened && (
          <PromptDialog
            ...
            onNavigate={(delta) => {
              const card = catalog?.items[activeIndex + delta];
        ...
        {access && config && (
          <AccessDialog
            ...
            onActivate={(key, expiresAt) => {
              store("brill_license", {
                key,
                expiresAt,
                activatedAt: new Date().toISOString(),
              });
              const saved = stored<Record<string, boolean>>(
                "bb_favs_" + key,
                {},
              );
              const merged = { ...saved, ...favorites };
              setFavorites(merged);
              store("bb_favs", merged);
              setPremium(true);
              setAccess(false);
              notify("גישת הפרימיום הופעלה");
            }}
            onLogout={async () => {
              try {
                await request("/api/session", { method: "DELETE" });
                const current = stored<{ key?: string } | null>(
                  "brill_license",
                  null,
                );
                if (current?.key) store("bb_favs_" + current.key, favorites);
                store("brill_license", null);
                setPremium(false);
                setAccess(false);
                notify("התנתקת בהצלחה");
              } catch (error) {
