...
async function readBody(req, max = 1024 * 1024) {
  ...
  if (req.body !== undefined && !Buffer.isBuffer(req.body)) {
    ...
    if (Buffer.isBuffer(req.body)) chunks.push(req.body);
      ...
      for await (const chunk of req) {
        size += chunk.length;
        if (size > max)
          throw Object.assign(new Error("הבקשה גדולה מדי"), { status: 413 });
        chunks.push(chunk);
      }
    body = Buffer.concat(chunks);
    if (req.headers["x-body-encoding"] === "gzip+json") {
      try {
        body = zlib.gunzipSync(body, { maxOutputLength: max });
      } catch {
        throw Object.assign(new Error("בקשה דחוסה לא תקינה או גדולה מדי"), {
          status: 413,
        });
      }
    }
  ...
  try {
    ...
    throw Object.assign(new Error("נתוני בקשה לא תקינים"), { status: 400 });
  }
}
