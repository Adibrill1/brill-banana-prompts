...
const API =
  "https://api.github.com/repos/Adibrill1/brill-banana-prompts/contents/";
const RAW =
  "https://raw.githubusercontent.com/Adibrill1/brill-banana-prompts/master/";
...
async function remote(file) {
  ...
  if (process.env.GITHUB_TOKEN) {
    ...
    if (result.status !== 200)
      throw new Error("לא ניתן לקרוא את הנתונים המעודכנים");
    revision = result.data.sha;
    if (result.data.content)
      text = Buffer.from(result.data.content, "base64").toString("utf8");
    else {
      // Pin the read to the file's current commit-independent blob to avoid stale raw CDN content.
      const response = await fetch(
        `https://api.github.com/repos/Adibrill1/brill-banana-prompts/git/blobs/${revision}`,
...
async function read(
  ...
  const job = remote(file)
    .then((value) => {
      entries.set(file, { value, at: Date.now() });
      return value;
    })
    .catch((error) => {
...
function invalidate(file = "state.json") {
  entries.delete(file);
}
